...
create or replace FUNCTION      ttec_f_grade_min (p_location hr.hr_locations_all.location_code%type,
...
create or replace FUNCTION      ttec_f_grade_min (p_location apps.hr_locations_all.location_code%type,
...
-- Program Name:  TTEC_F_GRADE_MIN
-- /* $Header: TTEC_F_GRADE_MIN.fnc 1.0  */
--
-- /*== START ================================================================================================*\
...
--
--      Desc: This function returns the minimum grade of salary depending on the location and job code of an employee.
--
...
P_MIN NUMBER(16,2) ;
P_LOCATION_SUBSTR VARCHAR(100 CHAR);
    ...
    */
    ...
    END;
      ...
      IF   p_job_information3 = 'NEX' AND p_business_group IN (325,326)  THEN
            ...
            p_min := ROUND( TO_NUMBER (p_min) * 2080 , -3);


      ELSIF p_attribute9 = 'NEX' AND p_business_group IN (1517)   THEN

            p_min := ROUND( TO_NUMBER (p_min) * 12, -3 );
