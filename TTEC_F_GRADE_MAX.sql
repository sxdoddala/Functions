...
code commentd by RXNETHI-ARGANO,19/05/23
create or replace FUNCTION      ttec_f_grade_max (p_location hr.hr_locations_all.location_code%type,
 ...
 */
 ...
 create or replace FUNCTION      ttec_f_grade_max (p_location apps.hr_locations_all.location_code%type,
...
-- Program Name:  TTEC_F_GRADE_MAX
-- /* $Header: TTEC_F_GRADE_MAX.fnc 1.0  */
--
-- /*== START ================================================================================================*\
...
--
--      Desc: This function returns the maximum grade of salary depending on the location and job code of an employee.
--
    ...
    */
    ...
    END;
      ...
      IF p_job_information3 = 'NEX' AND p_business_group IN (325,326)  THEN
            ...
            p_max := ROUND( TO_NUMBER (p_max) * 2080 , -3);


      ELSIF p_attribute9 = 'NEX' AND p_business_group IN (1517)   THEN

            p_max := ROUND ( TO_NUMBER (p_max) * 12, -3 );
